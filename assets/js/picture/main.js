	...
	GridFx.prototype._openItem = function(ev, item) { 
		...
		if( descriptionEl ) {
			...
			if(document.body.clientWidth<640){
				...
				//设置里面的内容 xxxxx
				//1.获取descriptionEl.innerHTML的值（数字），然后用接口调用这个图片的评论
				var data_num = descriptionEl.innerHTML;
				var temp =JSON.stringify($('.js_commit').find('.js_photo_detail').clone()) ;
				this.previewDescriptionEl.innerHTML = temp;
				$(this.previewDescriptionEl).html($('.js_commit').find('.js_photo_detail').clone());
				
				//点击发表
				$('.js_to_commit').click(function(){
					var $div_commit = $(this).parent('p').parent('div').parent('div');
					var content_ = $div_commit.find('textarea').val();
					var name = $div_commit.find('.js_name').val();
					var relation = $div_commit.find('.js_relation').val();
					//判断名字和内容是否填写了
					if(name){
						if(relation){
							if(content_){
								...
								console.log(tel.test(relation));
								if (tel.test(relation) || email.test(relation)){
									var temp = '<div class="result">'+content_+'<p class="photo_detail_p"><span>'+name+'</span><span>2018-07-02</span></p></div>';
	//											console.log('<div class="result">'+content_+'<p class="photo_detail_p"><span>'+$(".js_name").val()+'</span>');
